...
// ---------- Camera zoom ----------
// Controlled with the '+' / '-' keys (see keyboard()). This is the
// camera's distance from the scene, applied in drawScene() where the view
// used to be a fixed glTranslatef(0,0,-7.0). More negative = further away
// (zoomed out), less negative = closer (zoomed in).
float camDistance = -7.0f;
const float CAM_DISTANCE_MIN = -14.0f; // furthest zoom-out
const float CAM_DISTANCE_MAX = -3.0f;  // closest zoom-in
  ...
  void keyboard(unsigned char key, int x, int y)
{
    ...
    // ---------- Camera zoom: '+'/'=' zooms in, '-'/'_' zooms out ----------
    else if(key=='+' || key=='=')
    {
        camDistance += 0.6f;
...
void drawClouds()
{
 ...
 void sky()
        ...
        // Gradient sky dome: horizon color at the bottom, zenith color at the
        // top. Widened way past anything the frustum can see at any aspect
        // ratio or zoom level (was -40..40 / top 34.0, which the wide-screen
        // FOV could see past at the left/right edges, exposing the flat
        // glClearColor background there and creating a visible color seam).
        glBegin(GL_QUADS);
...
void roadside()
     ...
     for (float z = -38; z < WORLD_LENGTH; z += 9.0f)
	{
        ...
        // The RIGHT verge is widened out to x = 8.0 (was 5.0) so it runs all
        // the way under the shops/gas-station/hospital footprints on that
        // side — those buildings previously extended past x = 5.0, leaving a
        // gap of bare background between the grass and the building base.
        glBegin(GL_QUADS);
            glColor3ub(0,155,20);
        ...
        // The road surface itself spans x = -1.3 .. 1.3, and the houses /
        // shops / gas station / hospital all start at x >= ~2.1. Every
        // tuft below is kept inside x = 1.35 .. 2.0 (right) and
        // x = -4.3 .. -1.7 (left) — solidly on the grass verge, never on
        // the road and never reaching a building.
        ...
        for (float z = -10; z < WORLD_LENGTH; z += 1.2f)
        {
            ...
            // right verge tuft — kept right against the road edge, well
            // short of where the houses/shops begin
            glPushMatrix();
        ...
        // A second, irregular pass of darker tufts on both verges — varied
        // offset, rotation and size per tuft (driven by a couple of
        // differently-phased sine/cosine terms) so it reads as scruffy,
        // unordered wild grass rather than the evenly-spaced row above.
        // Same containment as the pass above: verges only, never the road,
        // never a building.
        for (float z = -8; z < WORLD_LENGTH; z += 1.7f)
        {
            ...
            glPushMatrix();
                ...
                glTranslatef(1.70f - rndB * 0.30f, z + rndA * 0.5f, 0.02f);
    ...
    void gasStation(float z, float xPos)
    {
        glPushMatrix();
            ...
            // Green landscaping surface along the station's base — widened
            // to run under the pumps as well as the kiosk, so the bottom
            // of the building reads as one real lawn instead of a thin strip.
            // Color matched to the roadside verge (0,155,20) so there's no
            // seam where the two greens meet.
    ...
    void shop(float z, int shopIdx, float xPos)
    {
        ...
        glPushMatrix();
            ...
            // the blue-tinted window glass above it. Color matched to the
            // roadside verge (0,155,20) so there's no seam where the two
            // greens meet.
            glColor3f(0.0f, 0.608f, 0.078f);
            glPushMatrix();
    ...
    void hospital(float z, float xPos)
    {
        glPushMatrix();
            ...
            // Green landscaping surface along the hospital's base — widened
            // out in front of the entrance so it reads as a real lawn
            // instead of a thin green line under the door. Color matched to
            // the roadside verge (0,155,20) so there's no seam where the two
            // greens meet.
            glColor3f(0.0f, 0.608f, 0.078f);
            glPushMatrix();
...
void drawScene() {
	...
	glTranslatef(0.0, 0.0, camDistance); //Move forward/back — zoom, controlled with +/- (see keyboard())
